...
{
	private const int LopcSaltBytes = 16;
	...
	public static LittleOPCAlgorithm CreateAlgorithmLOPC(string password, byte[] salt)
	{
		...
		{
			// LittleOPC is now a true 128-bit block/key algorithm on the C# crypto interface.
			lopc.Key = KDF.GetBytes(lopc.KeySize / 8);
			lopc.IV = KDF.GetBytes(lopc.BlockSize / 8);
		}
		return lopc;
	}
	...
	public void EncryptFileWithLittleOPC(string inputFile, string outputFile)
	{
		...
		byte[] salt = new byte[LopcSaltBytes];
