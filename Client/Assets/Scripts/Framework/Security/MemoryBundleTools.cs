...
namespace TwilightDreamOfMagical.CryptedMemoryBundle
{
	...
	public class Manager : MonoBehaviour
	{
		...
		private string VersionFileList = FrameworkConstant.ProtectedVersionFileListName;
		...
		private string VersionFileHashList = FrameworkConstant.ProtectedVersionFileHashListName;
		...
		public void CheckUpdateFiles()
		{
			if (string.IsNullOrEmpty(this.VersionFileList))
				this.VersionFileList = FrameworkConstant.ProtectedVersionFileListName;
			if (string.IsNullOrEmpty(this.VersionFileHashList))
				this.VersionFileHashList = FrameworkConstant.ProtectedVersionFileHashListName;
		...
		)
		{
			...
			ProcessPathsForRequiredFilesSync();
		...
		private class DownloadFileHashListState
		{
			...
			public string manifestText = null;
		}

		/// <summary>
		/// Download (if needed) the server file-hash list, cache it locally, and store the result in state.
		...
		/// </summary>
		private IEnumerator DownloadFileHashList(string fileHashUrl, string localManifestPath, DownloadFileHashListState state)
		{
			string text;

			// If not already on disk, fetch from server
			if (!FileReaderWriter.IsExist(localManifestPath))
			{
				var uwr = UnityWebRequest.Get(fileHashUrl);
				...
				File.WriteAllText(localManifestPath, text);
			}
			else
			{
				// Already cached locally: just read it
				text = File.ReadAllText(localManifestPath);
			}
		...
		/// </summary>
		private void ProcessPathsForRequiredFilesSync()
		{
			// 1. Build the URL and local path for the server hash list
			string fileHashUrl = Path.Combine(FrameworkConstant.ResourcesURL, this.VersionFileHashList);
			string localManifestPath = Path.Combine(this.ReadWriteRoot, this.VersionFileHashList);

			// 2. Kick off the coroutine and then busy-wait until it finishes
			var state = new DownloadFileHashListState();
			StartCoroutine(DownloadFileHashList(fileHashUrl, localManifestPath, state));

			// Busy-wait until download or error occurs
			while (!state.isDownloaded && !state.isError)
			{
				// NOTE: this blocks the main thread!
				// But this business process must be done this way!!!
			}

			// 3. Handle download error
			if (state.isError || string.IsNullOrEmpty(state.manifestText))
			{
			...
			var fileHashLookup = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
			string[] lines = state.manifestText.Trim().Replace("\r", "").Split('\n');
			...
			var AllAssetPath = new List<string>(this.bundleLoader.AssetBundleInfos.Keys);
			...
			{
				string assetPath = AllAssetPath[i];
				string localFile = Path.Combine(this.ReadWriteRoot, assetPath);

				if (!FileReaderWriter.IsExist(localFile))
					continue;

				byte[] fileBytes = FileReaderWriter.ReadExistingFile(localFile);
				string clientHash = FileHashTool.ComputeMD5(fileBytes);

				if (fileHashLookup.TryGetValue(assetPath, out string serverHash)
					&& clientHash.Equals(serverHash, StringComparison.OrdinalIgnoreCase))
				{
					// Match → schedule for removal
					BundleInfoNeedRemove.Add(assetPath);
				}
				else if (!fileHashLookup.ContainsKey(assetPath))
				{
					Debug.LogWarning($"No server hash for {assetPath}, will re-download.");
				}
				else
				{
					Debug.LogWarning($"Hash mismatch for {assetPath}: local={clientHash}, server={serverHash}");
				}
			}

			// 7. Remove all up-to-date entries in one pass
			for (int i = 0; i < BundleInfoNeedRemove.Count; i++)
			{
				this.bundleLoader.AssetBundleInfos.Remove(BundleInfoNeedRemove[i]);
			}
		}
		...
		public void BuildEncryptedBundle(UnityEditor.BuildTarget buildTarget)
		{
			...
			for (int i = 0; i < files.Length; i++)
			{
				...
				if (File.Exists(fileName))
				{
					...
					string fileHash = FileHashTool.ComputeMD5(fileBytes);
					fileHashs.Add(assetName + "|" + fileHash);
				}
			}
			...
			if (files.Length > 0)
			{
				...
				File.WriteAllText(PathUtil.BundleOutputPath + "/" + FrameworkConstant.ProtectedVersionFileListName, file_json, System.Text.Encoding.UTF8);
				File.WriteAllText(PathUtil.BundleOutputPath + "/" + FrameworkConstant.ProtectedVersionFileHashListName, file_hash_json, System.Text.Encoding.UTF8);
			}
