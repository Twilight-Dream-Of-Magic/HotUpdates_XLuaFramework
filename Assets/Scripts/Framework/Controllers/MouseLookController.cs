...
public class MouseLookController : MonoBehaviour
{
	...
	{
		// 1) 读取原始轴值，无任何平滑
		//    Use GetAxisRaw to remove Unity’s built‑in smoothing
		float rawX = Input.GetAxisRaw("Mouse X");
		float rawY = Input.GetAxisRaw("Mouse Y");

		// 2) 应用死区过滤，避免硬件噪声导致漂移
		//    Apply dead‑zone filtering to avoid drift
	...
	{
		// --- Yaw: 绕 Y 轴旋转刚体（角色水平转向） ---
		// --- Yaw: Rotate the Rigidbody around Y (horizontal turn) ---
		if (lookInput.x != 0f)
		{
		...
		// --- Pitch: 本地旋转 CameraPivot（上下仰角） ---
		// --- Pitch: Locally rotate the CameraPivot (vertical look) ---
		if (System.Object.ReferenceEquals(cameraPivot, null) && lookInput.y != 0f)
		{
			// 鼠标向上时 rawY 为正，因此要反向处理
			// Reverse input because moving mouse up should look up
