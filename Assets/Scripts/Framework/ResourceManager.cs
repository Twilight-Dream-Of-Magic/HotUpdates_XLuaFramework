...
public class ResourceManager : MonoBehaviour
{
	...
	{
		string versionFileListPath = Path.Combine(PathUtil.BuildResourcesPath, FrameworkConstant.VersionFileListName);
	...
	private IEnumerator LoadBundleAsync(string AssetName, Action<UnityEngine.Object> CallbackAction = null)
	{
		...
		*/
		...
		while (bundleFilePathStack.Count > 0)
		{
			...
			string currentBundlePath = Path.Combine(PathUtil.BuildResourcesPath, currentBundleName);
...
#endif
			StartCoroutine(LoadBundleAsync(AssetName, CallbackAction));
	}
